...
class E3HybridRouter(Router):
    ...
    def find_route(
        ...
        for iteration in range(self.hybrid_config.max_iterations):
            ...
            aco_res = self.aco.execute_iteration(origin_node_id, destination_node_id, context)
            bco_res = self.bco.execute_iteration(origin_node_id, destination_node_id, context)
            pso_res = self.pso.execute_iteration(origin_node_id, destination_node_id, context)
            
            total_expanded += (aco_res.nodes_expanded + bco_res.nodes_expanded + pso_res.nodes_expanded)
            ...
            def check_paths(res, source: str) -> None:
                nonlocal iter_best_cost, iter_best_nodes, iter_best_edges, iter_best_source
                for nodes, edges, cost in zip(res.path_nodes, res.path_edges, res.costs):
            ...
            if self.gbest_edges:
                if self.hybrid_config.share_bco_pso_to_aco:
                    self.aco.inject_global_best(self.gbest_nodes, self.gbest_edges, self.gbest_cost)
                if self.hybrid_config.share_gbest_to_bco:
                    self.bco.inject_global_best(self.gbest_nodes, self.gbest_edges, self.gbest_cost)
                if self.hybrid_config.share_gbest_to_pso:
                    self.pso.inject_global_best(self.gbest_nodes, self.gbest_edges, self.gbest_cost)
            ...
            if metrics:
                metrics.iteration_records.append(
                    E3HybridIterationMetrics(
                        ...
                        aco_contribution=n_aco / total_valid if total_valid > 0 else 0.0,
                        bco_contribution=n_bco / total_valid if total_valid > 0 else 0.0,
                        pso_contribution=n_pso / total_valid if total_valid > 0 else 0.0,
                    )
                )
