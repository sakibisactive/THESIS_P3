...
class PSORouter(Router):
    ...
    def _re_evaluate_bests(self, context: RoutingContext) -> None:
        ...
        for p in self.particles:
            if p.p_best_path_edges:
                p.p_best_cost = self._calculate_path_cost(
                    p.p_best_path_edges, context
                )

    def _calculate_path_cost(self, path_edges: list[str], context: RoutingContext) -> float:
    ...
    def _run_pso_iterations(
        ...
        for iteration in range(self.config.max_iterations):
            ...
            pbest_improvements = int(iter_result.custom_metrics.get("pbest_improvements", 0.0))

            if self.g_best_cost < query_best_cost:
                query_best_cost = self.g_best_cost
                conv_iter = iteration
            elif self.g_best_cost == query_best_cost and self.g_best_cost < float("inf"):
                identical_best_count += 1
                
            if metrics:
                self._record_iteration_metrics(
                    metrics, iteration, pbest_improvements
                )
    ...
    def _decode_path(
        ...
        def get_sorted_edges(node: str) -> list[Any]:
            edges = []
            for e in net.get_outgoing_edges(node):
    ...
    def _update_particles(self, context: RoutingContext) -> None:
        ...
        for p in self.particles:
            ...
            active_edges = set(p.X.keys()).union(p.p_best_X.keys()).union(self.g_best_X.keys())
            ...
            for edge in active_edges:
                ...
                if edge in p.X:
                    ...
                    x_current = self.scorer.heuristic(net.edges[edge], veh, net, incidents)
                else:
                    x_current = 0.0
                ...
                if edge in p.p_best_X:
                    ...
                    p_best_val = self.scorer.heuristic(net.edges[edge], veh, net, incidents) if edge in net.edges else 0.0
                    
                if edge in self.g_best_X:
                    g_best_val = self.g_best_X[edge]
                else:
                    g_best_val = self.scorer.heuristic(net.edges[edge], veh, net, incidents) if edge in net.edges else 0.0
    ...
    def _record_iteration_metrics(
        ...
        if valid_paths:
            ...
            if self.g_best_path_edges:
                ...
                exploring = sum(
                    1 for p in valid_paths if tuple(p.current_path_edges) != g_best_tuple
                )
